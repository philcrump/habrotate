...
tcp_config = ("127.0.0.1", 4533)
print "Connecting to rotctld.."
tcp_socket.connect(tcp_config)
loopcount = 0
update_rotator = 0
try:
	while True:
		...
		if loopcount == 0: #Set rotator on first loop
			...
			tcp_string = "P " + str(rotator_bearing) + " " + str(rotator_elevation)
			tcp_socket.send(tcp_string)
		else:
